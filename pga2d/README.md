...
* [**Pga2dLineCentral**](shared/src/main/scala/me/kright/gametools/pga2d/Pga2dLineCentral.scala): line passing through the center of coordinates, 2 fields. Dual to Pga2dVector

```scala
val line = Pga2dLine(a, b, c) // ax + by + c = 0
val centralLine = Pga2dLineCentral(a, b) // ax + by = 0
...
* [**Pga2dTransform**](shared/src/main/scala/me/kright/gametools/pga2d/Pga2dTransform.scala): a cached form of a normalized motor with the sandwich products precomputed as flat
  matrix coefficients (6 values in 2d: `cos` and `sin` of the rotation angle and the translations).
  ...
  `Pga2dTransform(motor)` renormalizes its argument, `Pga2dTransform.fromNormalized(motor)` trusts the caller;
  the result types narrow - `transform.sandwich(point)` is a `Pga2dPoint`.
  [**Pga2dProjectiveTransform**](shared/src/main/scala/me/kright/gametools/pga2d/Pga2dProjectiveTransform.scala) is the same cache for an arbitrary motor (plus `normSquare`,
  7 values), returning projective results scaled by `normSquare`.
