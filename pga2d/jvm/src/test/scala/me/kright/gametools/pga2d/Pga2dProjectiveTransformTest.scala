...
class Pga2dProjectiveTransformTest extends AnyFunSuiteLike with ScalaCheckPropertyChecks:
  ...
  test("sandwich and reverseSandwich for motor match motor") {
    forAll(Pga2dGenerators.anyMotors, Pga2dGenerators.anyMotors) { (motor, m2) =>
      val transform = Pga2dProjectiveTransform(motor)
  ...
  test("flat serialization round-trip") {
    assert(FlatDoubleSerializer.getSize[Pga2dProjectiveTransform] == Pga2dMotor.componentsCount + 7)

    forAll(Pga2dGenerators.anyMotors, Pga2dGenerators.anyMotors) { (m1, m2) =>
      val (a, b) = (Pga2dProjectiveTransform(m1), Pga2dProjectiveTransform(m2))
      val size = FlatDoubleSerializer.getSize[Pga2dProjectiveTransform]
      val arr = new Array[Double](size * 2)
      FlatDoubleSerializer.write(a, arr, offset = 0)
      FlatDoubleSerializer.write(b, arr, offset = size)
      assert(FlatDoubleSerializer.read[Pga2dProjectiveTransform](arr, offset = 0) == a)
      assert(FlatDoubleSerializer.read[Pga2dProjectiveTransform](arr, offset = size) == b)
    }
  }
  ...
  test("equalsWithEps") {
    forAll(Pga2dGenerators.anyMotors) { motor =>
      ...
      assert(transform.equalsWithEps(Pga2dProjectiveTransform(motor), 0.0))
      val shifted = Pga2dProjectiveTransform(motor.copy(wx = motor.wx + 1.0))
  ...
  test("id transform does not change objects") {
    forAll(Pga2dGenerators.projectivePoints) { p =>
      assert((Pga2dProjectiveTransform.id.sandwich(p) - p).norm < eps)
      assert((Pga2dProjectiveTransform.id.reverseSandwich(p) - p).norm < eps)
    }
  }
