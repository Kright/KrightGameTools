...
/** the hardcoded formulas for 4x4 matrices; the arguments may be any views */
object Matrix4d:
  def apply(array: Array[Double]): ArrayView2dFlat[Double] =
    ArrayView2dFlat(array, 4, 4)

  private def minor(m: ArrayView2d[Double], y1: Int, y2: Int, y3: Int, x1: Int, x2: Int, x3: Int): Double =
  ...
  private def minor(m: ArrayView2d[Double], yExclude: Int, xExclude: Int): Double =
    minor(m,
  ...
  private def require4x4(m: ArrayView2d[Double]): Unit = {
    require(m.w == 4)
    require(m.h == 4)
  }
   ...
   * @return self
   */
  def projectionCamera(tanX: Double, tanY: Double, near: Double, far: Double): ArrayView2dFlat[Double] =
   ...
   * @return self
   */
  def projectionCameraZ10(tanX: Double, tanY: Double, near: Double, far: Double): ArrayView2dFlat[Double] =
    val depth = far - near
    val sx = 1.0 / tanX
    val sy = 1.0 / tanY
    val sz = near / depth
   ...
   * @return self
   */
  def projectionCameraZ10(tanX: Double, tanY: Double, near: Double): ArrayView2dFlat[Double] =
    val sx = 1.0 / tanX
    val sy = 1.0 / tanY
    val sw = -near
    //@formatter:off
