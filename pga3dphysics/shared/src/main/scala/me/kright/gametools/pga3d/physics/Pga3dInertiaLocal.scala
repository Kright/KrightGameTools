...
final case class Pga3dInertiaLocal(mass: Double,
  ...
  private val invMass = 1.0 / mass
  private val invMryz = 1.0 / mryz
  private val invMrxz = 1.0 / mrxz
  private val invMrxy = 1.0 / mrxy
  ...
  override def invert(localInertia: Pga3dBivector): Pga3dBivector =
    Pga3dBivector(
      wx = localInertia.yz * invMass,
      wy = -localInertia.xz * invMass,
      wz = localInertia.xy * invMass,
      xy = localInertia.wz * invMrxy,
      xz = -localInertia.wy * invMrxz,
      yz = localInertia.wx * invMryz,
    )
