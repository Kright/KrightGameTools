...
* `scalagen` - the Scala generators, as a sub-package (named to avoid shadowing the root `scala` package):
  ...
  * `scalagen/pga3d` - the `Pga3dScalaAlgebra` object (class list, op list, naming for 3d), the op generators whose
    ...
    (the shared `ScalaTransformCodeGen` in `scalagen/common` generates the `Pga2dTransform` / `Pga3dTransform`
    classes - the motor sandwich cached as flat matrix coefficients; the 3d subclass adds the bivector moment block)
...
* the cached transforms (Pga3dTransform / Pga2dTransform) are generated by the shared ScalaTransformCodeGen; in 2d
  there is no bivector moment block and no normSquareI (a 2d motor has no pseudoscalar part, m * ~m = normSquare),
  and the rotation matrix stores only its two independent entries, named cos and sin of the rotation angle
