...
* The two former per-dimension `ScalaMultivectorSubClass` class bodies were byte-identical: every difference
  ...
  `scalagen/common`, injected with a `ScalaPgaAlgebra` instance (`Pga3dScalaAlgebra` / `Pga2dScalaAlgebra`) - not a
...
* Given that interface, the op generators split into two groups:
  * Shared once, in `scalagen/common/ops`, against `ScalaPgaAlgebra`: the 15 ops whose 2d/3d difference is naming or
  ...
  * Kept per-dimension, in `scalagen/pga3d/ops` and `scalagen/pga2d/ops`: ops whose math genuinely differs -
    `DefExpForBivector`, `DefInterpolation`, `DefLogForMotor`, `DefMotorAndRotorAxes`, `DefObjectMethodsForRotor`,
    `DefProjection`, `DefRenormalizedForMotor` (both dimensions) - plus the 3d-only ops (`DefBivectorSplit`,
    `DefRotorProjectToRotationInPlane`). Matrix generation (`ScalaMatrixCodeGen`) is shared in `scalagen/common`, derived from the grade-2 class fields.
