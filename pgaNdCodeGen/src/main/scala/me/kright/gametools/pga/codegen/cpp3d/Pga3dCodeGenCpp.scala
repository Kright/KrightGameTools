...
def runCppCodeGen(): Unit = {
  runCppCodeGen(RealFileSystem())
}

def runCppCodeGen(fs: GeneratedFileSystem): Unit = {
  new Pga3dCodeGenCpp(
    directory = Path.of("cpp/pga3d"),
    namespace = "pga3d",
  ).generateAll(fs)

  CustomAmalgamate.fuseMath(fs)
  CustomAmalgamate.fusePhysicsWithMath(fs)
}
...
class Pga3dCodeGenCpp(val directory: Path,
  ...
  def generateAll(fs: GeneratedFileSystem): Unit = {
    fs.write(generateStructForwardDeclarations())
    fs.write(generateStructHeadersAggregation())
    ...
    ops.foreach(op => fs.write(op))
    fs.write(generatePga3d(ops))
  }
  ...
  private def generateStructHeader(cls: CppSubclass): String = {
    ...
    codeGen.myHeader(includes, codeGen.generatorName(this))
      ...
      codeGen.struct(cls.name) {
        codeGen(
          structBodyParts
            ...
            .map(sc => s"// generated by ${codeGen.generatorName(sc.sourceGenerator)}\n\n${sc.structCode}")
            .mkString("\n\n")
        )
      }
