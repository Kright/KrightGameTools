package me.kright.gametools.pga.codegen.scalagen.pga3d.ops

import me.kright.gametools.ga.PGA3
import me.kright.gametools.pga.codegen.common.{FormulaTemplate, SharedFormulas}
import me.kright.gametools.pga.codegen.scalagen.common.{GeneratedCode, MultivectorUnaryOp}
import me.kright.gametools.pga.codegen.scalagen.pga3d.Pga3dScalaAlgebra

object DefRenormalizedForMotor:
  def apply()(using pga3: PGA3): MultivectorUnaryOp = MultivectorUnaryOp { (cls, v) =>
    GeneratedCode { code =>
      val self = cls.self

      if (cls == Pga3dScalaAlgebra.motor) {
        code(
          s"""
             |/**
             | * see [[https://arxiv.org/abs/2206.07496]], page 14
             | * and [[https://bivector.net/PGAdyn.pdf]], page 42
             | */
             |def renormalized: Pga3dMotor =""".stripMargin)
        code.block {
          code(FormulaTemplate.renderScala(SharedFormulas.motorRenormalized, Pga3dScalaAlgebra.typeNamePrefix))
          code(cls.makeConstructor(SharedFormulas.motorRenormalizedResult(self)))
        }
      }
    }
  }
