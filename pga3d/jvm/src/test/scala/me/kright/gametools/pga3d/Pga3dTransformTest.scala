...
class Pga3dTransformTest extends AnyFunSuiteLike with ScalaCheckPropertyChecks:
  ...
  test("flat serialization round-trip") {
    assert(FlatDoubleSerializer.getSize[Pga3dTransform] == Pga3dMotor.componentsCount + 26)

    forAll(Pga3dGenerators.anyMotors, Pga3dGenerators.anyMotors) { (m1, m2) =>
      val (a, b) = (Pga3dTransform(m1), Pga3dTransform(m2))
  ...
  test("equalsWithEps") {
    forAll(Pga3dGenerators.anyMotors) { motor =>
      ...
      assert(transform.equalsWithEps(Pga3dTransform(motor), 0.0))
      val shifted = Pga3dTransform(motor.copy(wx = motor.wx + 1.0))
